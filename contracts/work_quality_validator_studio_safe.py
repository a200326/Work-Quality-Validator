    ...
    def submit_work(self, contract_id: int, delivery_url: str) -> None:
        ...
        def evaluate() -> str:
            ...
            prompt = (
                ...
                "clearly satisfied by the actual content above. issues lists, "
                "in a few words each, every criterion that is not satisfied. "
                "If everything is satisfied, issues must be an empty list."
            )
        ...
        raw_result = gl.eq_principle.prompt_comparative(
            evaluate,
            principle=(
                "Both responses are JSON objects with a numeric 'score' "
                "(0-100) and a list of 'issues'. Treat them as equivalent "
                "only if: (1) the two scores differ by 15 points or less, "
                "AND (2) the two issues lists point at substantially the "
                "same shortcomings in the work relative to the rubric "
                "(not necessarily identical wording, but the same "
                "underlying problems — or both empty if the work is "
                "genuinely rubric-compliant). A leader score that is "
                "clearly more lenient or more harsh than an independent "
                "re-evaluation of the same content against the same "
                "rubric is NOT equivalent, even if both are valid JSON."
            ),
        )
